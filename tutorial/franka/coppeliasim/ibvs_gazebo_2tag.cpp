...
main( int argc, char **argv )
{
  ...
  try
  {
    ...
    ros::NodeHandle _n;
    ros::Publisher point_pub = _n.advertise<geometry_msgs::Pose>("/ndt2/camera/tag_pose", 0);
    ros::Publisher flag_pb = _n.advertise<std_msgs::Bool>("/ndt2/camera/tag_flag", 0);
 		ros::Publisher _vs_pub = _n.advertise< std_msgs::Float64MultiArray >("/ndt2/tag_pos_px_corner", 0);
 		ros::Publisher _vs_ee_pub = _n.advertise< std_msgs::Float64MultiArray >("/ndt2/tag_pos_px_corner_ee", 0);

    ros::Subscriber ee_sub = _n.subscribe("/ndt2/ee_pos_corner", 1000, ee_pointsCallback);
    ...
    //
    vpDetectorAprilTag detector_2( tagFamily );
    detector_2.setAprilTagPoseEstimationMethod( poseEstimationMethod );
    detector_2.setDisplayTag( display_tag );
    detector_2.setAprilTagQuadDecimate( opt_quad_decimate );
    //
    ...
    vpHomogeneousMatrix cdMo( vpTranslationVector( 0.0, -0.10, opt_tagSize * 4 - 0.055 ), //era 0.085
    ...
    while ( !final_quit )
    {
      ...
      std::vector< vpHomogeneousMatrix > cMo_vec, cMo_vec_2;
      detector.detect( I, opt_tagSize, cam, cMo_vec );
      detector_2.detect( I, opt_tagSize_2, cam, cMo_vec_2 );
      //  cMo = cMo_vec[0];
      //  std::cout<<detector.getPose(0, opt_tagSize, cam, cMo);
      //  std::cout<<"\n"<<cMo<<"\n"<<std::endl;

      // cout<<"Id message: "<<detector_2.getMessage()[0]<<endl;
      // cout<<"Id message: "<<detector_2.getMessage()[1]<<endl;
      ...
      // if ( cMo_vec.size() == 1 )       // Only one tag is detected
      if ( cMo_vec.size() < 3 )           // Only one tag is detected or 2 at max
      {
        found.data = true;
        cMo = cMo_vec[0];
        ...
        // Get tag corners
        if(detector.getMessage()[0] == "36h11 id: 0"){
          corners = detector.getPolygon( 0 );
        ...
        if (cMo_vec.size() > 1){
          std::vector< vpImagePoint > corners_2;
          if(detector_2.getMessage()[0] == "36h11 id: 42"){
            // cout<<"selected tag 1"<<endl;
            corners_2 = detector_2.getPolygon( 0 );
            // for (int i=0; i<corners_2.size();i++){
            //   cout << "vector[ "<<i<<" ]: " <<corners_2[i] << endl;
            // }
          }
          else if (detector_2.getMessage()[1] == "36h11 id: 42"){
            // cout<<"selected tag 2"<<endl;
            corners_2 = detector_2.getPolygon( 1 );
            // for (int i=0; i<corners_2.size();i++){
            //   cout << "vector[ "<<i<<" ]: " <<corners_2[i] << endl;
            // } 
          }
          for ( size_t i = 0; i < corners_2.size(); i++ )
          {
            std::stringstream ss;
            ss << i;
            // Display current point indexes
            vpDisplay::displayText( I, corners_2[i] + vpImagePoint( 15, 15 ), ss.str(), vpColor::red );
          }
          std_msgs::Float64MultiArray corners_px_ee;
          corners_px_ee.data.resize(9);
          corners_px_ee.data[0] = 8;
          corners_px_ee.data[1] = corners_2[0].get_i();
          corners_px_ee.data[2] = corners_2[0].get_j();
          corners_px_ee.data[3] = corners_2[1].get_i();
          corners_px_ee.data[4] = corners_2[1].get_j();
          corners_px_ee.data[5] = corners_2[2].get_i();
          corners_px_ee.data[6] = corners_2[2].get_j();
          corners_px_ee.data[7] = corners_2[3].get_i();
          corners_px_ee.data[8] = corners_2[3].get_j();
          _vs_ee_pub.publish(corners_px_ee);
        }   
        ...
        if (_EE_corner_px_pos(0,0)>1 && _EE_corner_px_pos(0,1)>1 && _EE_corner_px_pos(0,2)>1 && _EE_corner_px_pos(0,3)>1 && cMo_vec.size() > 1) {
          ...
          // vpDisplay::displayPoint(I, vpImagePoint(476.198, 343.666 ), vpColor::green, 2);
          // vpDisplay::displayPoint(I, vpImagePoint(476.272, 380.96 ), vpColor::green, 2);
          // vpDisplay::displayPoint(I, vpImagePoint(439.771, 380.96 ), vpColor::green, 2);
          // vpDisplay::displayPoint(I, vpImagePoint(439.642, 343.861 ), vpColor::green, 2);
          
          // vpDisplay::displayText( I, vpImagePoint(476.198, 343.666 ) + vpImagePoint( 10, 10 ), stringstream("0").str(), vpColor::green );
          // vpDisplay::displayText( I, vpImagePoint(476.272, 380.96 ) + vpImagePoint( 10, 10 ), stringstream("1").str(), vpColor::green );
          // vpDisplay::displayText( I, vpImagePoint(439.771, 380.96 ) + vpImagePoint( 10, 10 ), stringstream("2").str(), vpColor::green );
          // vpDisplay::displayText( I, vpImagePoint(439.642, 343.861 ) + vpImagePoint( 10, 10 ), stringstream("3").str(), vpColor::green ); 
          
          vpDisplay::displayPoint(I, vpImagePoint(473.293, 343.198 ), vpColor::green, 2);
          vpDisplay::displayPoint(I, vpImagePoint(473.799, 377.764 ), vpColor::green, 2);
          vpDisplay::displayPoint(I, vpImagePoint(440.41, 379.205 ), vpColor::green, 2);
          vpDisplay::displayPoint(I, vpImagePoint(440.05, 344.413 ), vpColor::green, 2);
          
          vpDisplay::displayText( I, vpImagePoint(473.293, 343.198 ) + vpImagePoint( 10, 10 ), stringstream("0").str(), vpColor::green );
          vpDisplay::displayText( I, vpImagePoint(473.799, 377.764 ) + vpImagePoint( 10, 10 ), stringstream("1").str(), vpColor::green );
          vpDisplay::displayText( I, vpImagePoint(440.41, 379.205 ) + vpImagePoint( 10, 10 ), stringstream("2").str(), vpColor::green );
          vpDisplay::displayText( I, vpImagePoint(440.05, 344.413 ) + vpImagePoint( 10, 10 ), stringstream("3").str(), vpColor::green ); 
          

        }  
        if (print ==true && cMo_vec.size() > 1){
